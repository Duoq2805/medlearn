...
export default function SymptomCheckerPage() {
  ...
  const recentSearches = ['Cough + Fever + Dyspnea', 'Chest Pain + Nausea', 'Headache + Fatigue'];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="card-neumorphic p-6">
                ...
                <button disabled className={`btn-neumorphic-primary py-3 px-8 w-full flex items-center justify-center gap-2 ${selectedSymptoms.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}>
              ...
              <div className="card-neumorphic p-6">
                ...
                {selectedSymptoms.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {['Pneumonia', 'Bronchitis', 'Upper Respiratory Infection'].map((disease, i) => (
                      <div key={i} className="depth-layer-1 rounded-2xl p-4 flex items-center justify-between">
                        <div>
                          <p className="font-semibold text-sm text-[var(--text-primary)]">{disease}</p>
                          <p className="text-xs text-[var(--text-secondary)]">Confidence: {90 - i * 15}%</p>
                        </div>
                        <Link to={`/disease/${disease.toLowerCase().replace(/\s/g, '-')}`} className="text-xs text-[var(--accent-primary)] hover:underline">View</Link>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="space-y-6">
              ...
              <div className="card-neumorphic p-6">
                ...
                <div className="space-y-2">
                  <button disabled className="btn-neumorphic-secondary py-2 px-4 w-full text-sm flex items-center justify-center gap-2 opacity-50 cursor-not-allowed">
                  ...
                  <Link to="/explorer" className="btn-neumorphic-primary py-2 px-4 w-full text-sm flex items-center justify-center gap-2">
